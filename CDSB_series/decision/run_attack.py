...
if __name__ == '__main__':
    ...
    if os.path.exists(trainfeaturepath):
        ...
        logger.debug(call("python3 " + join(BASE_DIR,"extract.py") + " " +args.train, shell = True))
    ...
    if os.path.exists(testfeaturepath):
        ...
        if args.num:
            #print("1111111111111")
            logger.debug(call("python3 " + join(BASE_DIR,"extract.py") + " " +args.test +" -num "+ args.num, shell = True))
        else:
            #print("2222222222222")
            logger.debug(call("python3 " + join(BASE_DIR,"extract.py") + " " +args.test, shell = True))


    if os.path.exists(modelpath):
        logger.debug("skip train model")
    else:
        logger.debug(call("python3 " + join(BASE_DIR, "train.py") + " " +trainfeaturepath, shell = True))
        logger.info("train decision model finished")
    
    logger.info(call("python3 " + join(BASE_DIR, "evaluate.py") + " " +testfeaturepath + " -m "+modelpath, shell = True))
