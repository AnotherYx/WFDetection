...
void accuracy(float** trainfeat, float** testfeat, int* trainfeatclasses, int* testfeatclasses, int trainlen, int testlen, float* weight) {
	...
	for (int is = 0; is < testlen; is++) {
		...
		FILE * flog;
		flog = fopen("flearner-other.results", "a");
		fprintf(flog, "%d", trueclass);

		int CLASS_NUM = atoi(d["CLOSED_SITENUM"].c_str());//100
		if (atoi(d["OPEN_INSTNUM"].c_str()) > 0) CLASS_NUM += 1;//101
...
void read_filelist(float ** feat, int * featclasses, int featlen, string fname) {
	...
	while (fread.peek() != EOF) {
		...
		str = rstr.substr(found+1);////0-32-86.cell.cellkNN
		str = str.substr(0, str.find_first_of("."));//0-32-86
		//closed or open?
		if (str.find("-") != string::npos) {
			//this means closed
			string str1 = str.substr(0, str.find_first_of("-")); //0
			string str2 = str.substr(str.find_first_of("-")+1); //32-86
...
int main(int argc, char** argv) {
	...
	string testlist = data_path + string(test_name) +'/' + "testlist-other";
