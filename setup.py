...
def get_extensions_pycocotools():
    this_dir = path.dirname(path.abspath(__file__))
    ext_modules = [
        Extension(
            'pycocotools._mask',
            sources=[path.join(this_dir,'pycocotools/maskApi.c'), path.join(this_dir,'pycocotools/_mask.pyx')],
            include_dirs=[np.get_include(), path.join(this_dir,'pycocotools')],
...
if __name__ == "__main__":
    ...
    setup(
        ...
        version='2.0.4',
        ext_modules=get_extensions_pycocotools()
    )
