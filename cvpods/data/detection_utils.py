...
def check_trace_size(dataset_dict, image):  
    """
    Raise an error if the image does not match the size specified in the dict.
    """
    if "width" in dataset_dict or "height" in dataset_dict:
...
def load_trace(file_name, format=None): 
    """
    Read an trace into the given format(direction or timestamp or both).
    Will apply rotation and flipping if the image has such exif information.
    Args:
        file_name (str): image file path
        format (str): one of the supported image modes in  'dir' or 'tim' or 'both'.
    Returns:
        trace (np.ndarray): an trace in the given format, which is +1 or -1 for
            'dir' mode; float for 'tim' mode; (float, +-1) for 'both' mode.
    """
    # file_name = file_name[:-6] + ".npy"
