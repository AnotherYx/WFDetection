...
def build_train_loader(cfg):
    ...
    #logger.info(f"TransformGens used: {transform_gens} in training") #ignore [WFD 2022]
    dataset = build_dataset(
        cfg, cfg.DATASETS.TRAIN, transforms=transform_gens, is_train=True
    )

    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN
    #logger.info("Using training sampler {}".format(sampler_name))#ignore [WFD 2022]
...
def build_test_loader(cfg):
    ...
    logger = logging.getLogger(__name__)
    # logger.info(f"TransformGens used: {transform_gens} in testing")#ignore [WFD 2022]
