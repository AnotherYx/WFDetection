...
def _create_grid_offsetsTrace(size, stride, offset, device): 
    _, grid_width = size
    ...
    shifts_y = torch.tensor([0])
    shifts_y = shifts_y.type_as(shifts_x)
    shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
    shift_x = shift_x.reshape(-1)
    shift_y = shift_y.reshape(-1)
    return shift_x, shift_y
...
class WFDAnchorGenerator(nn.Module):
    """
    For a set of trace sizes and feature maps, computes a set of anchors.
    """
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        sizes (list[list[int]]): sizes[i] is the list of anchor sizes to use
            for the i-th feature map. If len(sizes) == 1, then the same list of
            anchor sizes, given by sizes[0], is used for all feature maps. Anchor
            sizes are given in absolute lengths in units of the input trace;
            they do not dynamically scale if the input trace size changes.
        aspect_ratios (list[list[float]]): aspect_ratios[i] is the list of
            anchor aspect ratios to use for the i-th feature map. If
            len(aspect_ratios) == 1, then the same list of anchor aspect ratios,
            given by aspect_ratios[0], is used for all feature maps.
    ...
    def grid_anchors(self, grid_sizes):
        anchors = []
        for size, stride, base_anchors in zip(grid_sizes, self.strides, self.cell_anchors):
            shift_x, shift_y = _create_grid_offsetsTrace(size, stride, self.offset, base_anchors.device)
            shifts = torch.stack((shift_x, shift_y, shift_x, shift_y), dim=1)
    ...
    def generate_cell_anchors(self, sizes=(32, 64, 128, 256, 512), aspect_ratios=(0.5, 1, 2)):
        ...
        Args:
            sizes (tuple[float]): Absolute size of the anchors in the units of the input
                trace (the input received by the network, after undergoing necessary scaling).
                The absolute size is given as the side length of a box.
            aspect_ratios (tuple[float]]): Aspect ratios of the boxes computed as box
                height / width.
    ...
    def forward(self, features):
        ...
        Returns:
            list[list[Boxes]]: a list of #trace elements. Each is a list of #feature level Boxes.
                The Boxes contains anchors of this trace on the specific feature level.
        """
        num_traces = len(features[0])
        grid_sizes = [feature_map.shape[-2:] for feature_map in features]
        anchors_over_all_feature_maps = self.grid_anchors(grid_sizes)
        ...
        for anchors_per_feature_map in anchors_over_all_feature_maps:
            boxes = Boxes(anchors_per_feature_map)
            anchors_in_trace.append(boxes)

        anchors = [copy.deepcopy(anchors_in_trace) for _ in range(num_traces)]
        return anchors
