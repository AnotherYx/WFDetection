...
__all__ = ["Box2BoxTransform", "Box2BoxTransformTrace", "Box2BoxTransformRotated"] 
...
class Box2BoxTransformTrace(object): 
    """
    The box-to-box transform defined in R-CNN. The transformation is parameterized
    by 2 deltas: (dx, dw). The transformation scales the box's width and height
    by exp(dw), 0 and shifts a box's center by the offset (dx * width, 0).
    """
    ...
    def __init__(
        ...
        """
        Args:
            weights (2-element tuple): Scaling factors that are applied to the
                (dx, dw) deltas. In Fast R-CNN, these were originally set
                such that the deltas have unit variance; now they are treated as
                hyperparameters of the system.
            scale_clamp (float): When predicting deltas, the predicted box scaling
                factors (dw and dh) are clamped such that they are <= scale_clamp.
    ...
    def get_deltas(self, src_boxes, target_boxes):
        """
        Get box regression transformation deltas (dx, dw) that can be used
        to transform the `src_boxes` into the `target_boxes`. That is, the relation
        ``target_boxes == self.apply_deltas(deltas, src_boxes)`` is true (unless
        any delta is too large and is clamped).
    ...
    def apply_deltas(self, deltas, boxes):
        """
        Apply transformation `deltas` (dx, dw) to `boxes`.

        Args:
            deltas (Tensor): transformation deltas of shape (N, k*3), where k >= 1.
                deltas[i] represents k potentially different class-specific
                box transformations for the single box boxes[i].
            boxes (Tensor): boxes to transform, of shape (N, 4)
        """
        ...
        if self.add_ctr_clamp:
            dx_width = torch.clamp(dx_width,
                                   max=self.ctr_clamp,
                                   min=-self.ctr_clamp)
            # dy_height = torch.clamp(dx_width,
            #                         max=0,
            #                         min=0)
        dy_height = torch.zeros_like(dx_width)
        dw = torch.clamp(dw, max=self.scale_clamp)
        # dh = torch.clamp(dw, max=0,  min=0)
        dh = torch.zeros_like(dw)
